...
function commitTagChanges(newState, firstRender) {
  ...
  const tagUpdates = {
    ...
    linkTags: updateTags(_constants.TAG_NAMES.LINK, [...(priority?.links ?? []), ...(links ?? [])]),
    metaTags: updateTags(_constants.TAG_NAMES.META, [...(priority?.meta ?? []), ...(meta ?? [])]),
    noscriptTags: updateTags(_constants.TAG_NAMES.NOSCRIPT, noscript ?? []),
    scriptTags: updateTags(_constants.TAG_NAMES.SCRIPT, [...(priority?.script ?? []), ...(script ?? [])]),
